...
import { Heart, ShoppingCart } from "lucide-react"; // Imported ShoppingCart
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="-translate-x-35">
            <Link to="/" className="text-3xl font-bold text-[#F5EFE7]">
              Rentora
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center gap-8">
            <Link to="/home" className="text-[#F5EFE7] hover:text-[#D8C4B6] transition">
              Home
            </Link>
            <Link to="/products" className="text-[#F5EFE7] hover:text-[#D8C4B6] transition">
              Products
            </Link>
          ...
          <div className="flex items-center gap-6 translate-x-35">
            ...
            {user ? (
              <div className="relative ml-2" ref={dropdownRef}>
                ...
                >
              <div className="w-10 h-10 rounded-full bg-[#D8C4B6] text-[#213555] flex items-center justify-center font-bold overflow-hidden">
                  {user.profileImage ? ( 
                      /* Show their photo if they uploaded one */
                      <img 
                          src={`http://localhost:5200${user.profileImage}`} 
                          alt="Profile" 
                          className="w-full h-full object-cover"
                      />
                  ) : (
                      /* Fallback to the first letter of their email if no photo exists */
